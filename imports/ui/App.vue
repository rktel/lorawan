                        ...
                        <div style="height: 100%; width: 100%">
                            ...
                            <LMap style="height: 90%; width: 100%" :zoom="zoom" :center="center" @update:zoom="zoomUpdated" @update:center="centerUpdated" @update:bounds="boundsUpdated">
                                ...
                                <template v-if="data.length>0">
                                    <LCircleMarker v-for="item in data" :key="item._id" :lat-lng="[item.latitud, item.longitud]" :radius="circle.radius" :color="circle.color">
                                        <LPopup>
                                            {{localTime(item.datetime)}} -{{item.place}}
                                        </LPopup>
                                    </LCircleMarker>
                                </template>
                            </LMap>
